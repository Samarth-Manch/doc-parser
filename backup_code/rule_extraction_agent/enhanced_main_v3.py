"""
Enhanced Rule Extraction Agent v3 - Improved version with better rule generation.

Key improvements in v3:
1. EXT_VALUE rules for 13 expected external data value fields
2. All 6 OCR rules including Aadhaar Back and CIN
3. Better VERIFY ordinal mappings from reference
4. Consolidated MAKE_DISABLED rules
"""
...
class EnhancedRuleExtractionAgentV3:
    """Enhanced rule extraction agent v3 with improved rule generation."""
    ...
    OCR_VERIFY_CHAINS = {
        ...
        'CIN': None,  # CIN has no VERIFY chain
        'AADHAR_IMAGE': None,
        'AADHAR_BACK_IMAGE': None,
    }

    # Field name patterns for OCR detection - enhanced for v3
    OCR_FIELD_PATTERNS = {
        ...
        r'aadhaar?\s*back|back\s*aadhaar?|aadhaar?\s*back\s*image|aaadhar\s*back': ('AADHAR_BACK_IMAGE', None),
    ...
    # EXT_VALUE field patterns - from reference analysis
    EXT_VALUE_FIELDS = {
        'choose the group of company': {
            'ddType': 'COMPANY_CODE_PURCHASE_ORGANIZATION',
            'depends_on': 'company code'
        },
        'company and ownership': {
            'ddType': 'COMPANY_CODE_PURCHASE_ORGANIZATION',
            'depends_on': 'choose the group of company'
        },
        ...
        'group key/corporate group': {
            'ddType': 'VC_VENDOR_TYPES',
            'depends_on': 'account group/vendor type'
        },
        'vendor country': {
            'ddType': 'COUNTRY',
            'depends_on': None
        ...
        'region (import)': {
            'ddType': 'COUNTRY',
            'depends_on': 'vendor country'
        },
        'please choose the option': {
            'ddType': 'BANK_OPTIONS',
            'depends_on': 'vendor domestic or import'
        },
        'bank country (import)': {
            'ddType': 'COUNTRY',
            'depends_on': None
    ...
    def process(self) -> Dict:
        ...
        self.log("Starting enhanced rule extraction v3...")
        ...
        # Step 4: Generate EXT_VALUE rules - NEW in v3
        ...
        # Step 5: Generate MAKE_DISABLED rules
        ...
        # Step 6: Generate CONVERT_TO rules
        ...
        # Step 7: Link OCR to VERIFY rules
    ...
    def _generate_ocr_verify_rules(self):
        """Generate OCR and VERIFY rules - Enhanced for v3 with all 6 OCR rules."""
        ocr_generated = set()
        ...
        for field in self.fields:
            ...
            for pattern, (ocr_source, verify_dest_pattern) in self.OCR_FIELD_PATTERNS.items():
                if re.search(pattern, combined, re.I) and ocr_source not in ocr_generated:
    ...
    def _generate_ext_dropdown_rules(self):
        ...
        for field in self.fields:
            if field.field_type in ext_types:
                ...
                if field.name.lower() in self.EXT_VALUE_FIELDS:
                    continue
    ...
    def _generate_ext_value_rules(self):
        """Generate EXT_VALUE rules for external data value fields - NEW in v3."""
        for field in self.fields:
            name_lower = field.name.lower().strip()

            # Check if this field needs EXT_VALUE
            for pattern, config in self.EXT_VALUE_FIELDS.items():
                if pattern in name_lower or name_lower in pattern:
                    # Find parent field if dependency exists
                    source_ids = [field.id]
                    if config.get('depends_on'):
                        parent_field = self.match_field(config['depends_on'])
                        if parent_field:
                            source_ids = [parent_field.id]
    ...
    def _build_ext_value_params(self, field: FieldInfo, config: Dict) -> str:
        ...
        # Build a simplified params structure
        params_obj = [{
            "conditionList": [{
                "ddType": [dd_type],
                "criterias": [],
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Rule Extraction Agent v3')
