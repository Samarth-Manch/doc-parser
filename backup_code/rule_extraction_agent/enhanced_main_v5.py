"""
Enhanced Rule Extraction Agent v5 - Complete Implementation

Key improvements in v5 based on eval report and self-heal instructions:
1. Add ALL missing rule types: EXECUTE, VALIDATION, COPY_TO, DELETE_DOCUMENT, UNDELETE_DOCUMENT
2. Add SESSION_BASED_* visibility/mandatory rules
3. Fix ALL OCR → VERIFY chains (was 4 missing)
4. Add COPY_TO_DOCUMENT_STORAGE_ID for file upload fields
5. Better visibility/mandatory rule generation from logic text
6. Improved field matching with fuzzy matching
7. Fix data type issues (str vs dict handling)
"""
...
class EnhancedRuleExtractionAgentV5:
    """Enhanced rule extraction agent v5 with complete implementation."""
    ...
    # Session-based rule patterns (for approver views)
    ...
    def process(self) -> Dict:
        ...
        self.log("Starting enhanced rule extraction v5...")
        ...
        # Step 9: Generate VALIDATION rules (NEW in v5)
        ...
        # Step 13: Generate SESSION_BASED rules (NEW in v5)
        ...
        # Step 14: Generate COPY_TO_DOCUMENT_STORAGE_ID rules (NEW in v5)
    ...
    def _generate_validation_rules(self):
        """Generate VALIDATION rules (NEW in v5)."""
        for field in self.fields:
            logic = (field.logic or '').lower()
    ...
    def _generate_copy_to_rules(self):
        """Generate COPY_TO rules (NEW in v5)."""
        if not self.intra_panel_refs:
            return
    ...
    def _generate_execute_rules(self):
        """Generate EXECUTE rules for complex expressions (NEW in v5)."""
        for field in self.fields:
            logic = (field.logic or '').lower()

            # Check for expression patterns
            if re.search(r'expr-eval|mvi\s*\(|mm\s*\(|expression|calculate', logic, re.I):
    ...
    def _generate_document_rules(self):
        """Generate DELETE_DOCUMENT and UNDELETE_DOCUMENT rules (NEW in v5)."""
    ...
    def _generate_session_based_rules(self):
        """Generate SESSION_BASED visibility/mandatory rules (NEW in v5)."""
        for field in self.fields:
            logic = (field.logic or '').lower()
    ...
    def _generate_document_storage_rules(self):
        """Generate COPY_TO_DOCUMENT_STORAGE_ID rules for file fields (NEW in v5)."""
        for field in self.file_upload_fields:
            rule = self._create_rule(
...
def run_extraction(
    ...
    agent = EnhancedRuleExtractionAgentV5(
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        print("Usage: python enhanced_main_v5.py <schema_path> <intra_panel_path> <output_path> [report_path]")
        sys.exit(1)
