"""
Enhanced Rule Extraction Agent v8 - Complete Implementation with All Fixes

CRITICAL FIXES in v8 based on eval report v2 and self-heal instructions v3:

1. VERIFY destinationIds MUST use FIELD IDs in ordinal positions:
   - The array is indexed by ordinal (ordinal 1 = index 0)
   - The VALUE at each position is the FIELD ID (not the ordinal number!)
   - Use -1 for ordinals without mapped fields

2. Fix ALL OCR-VERIFY chains:
   - PAN_IMAGE -> PAN_NUMBER
   - GSTIN_IMAGE -> GSTIN
   - CHEQUEE -> BANK_ACCOUNT_NUMBER
   - MSME -> MSME_UDYAM_REG_NUMBER
   - CIN -> CIN_ID
   - AADHAR_IMAGE -> AADHAR_IMAGE (no VERIFY, just OCR)
   - AADHAR_BACK_IMAGE -> AADHAR_BACK_IMAGE (no VERIFY, just OCR)

3. Add missing rule types:
   - CONVERT_TO: 21 expected (only 5 generated)
   - COPY_TO: 12 expected (only 3 generated)
   - VALIDATION: 18 expected (only 2 generated)
   - MAKE_DISABLED: 5 expected (only 1 generated) - consolidated

4. Fix rule counts:
   - MAKE_VISIBLE: 18 expected (14 generated)
   - MAKE_INVISIBLE: 19 expected (14 generated)
   - EXT_VALUE: 13 expected (32 generated - too many!)
   - EXT_DROP_DOWN: 20 expected (18 generated)

5. Add special rules:
   - COPY_TXNID_TO_FORM_FILL
   - SET_DATE for Created On
   - COPY_TO for Created By Name
   - COPY_TO_TRANSACTION_ATTR1/3
   - COPY_TO_GENERIC_PARTY_EMAIL/NAME
...
class EnhancedRuleExtractionAgentV8:
    """Enhanced rule extraction agent v8 with all fixes from eval report."""

    # VERIFY ordinal mappings from Rule-Schemas.json
    # CRITICAL: destinationIds array is indexed by ordinal (ordinal 1 = index 0)
    # The VALUE at each position is the FIELD ID
    VERIFY_ORDINALS = {
        'PAN_NUMBER': {
    ...
    OCR_ORDINALS = {
        ...
        'AADHAR_IMAGE': {
            1: 'aadharNumber', 2: 'name', 3: 'gender', 4: 'dob',
        },
        'AADHAR_BACK_IMAGE': {
        ...
        'CIN': {
            1: 'cinNumber', 2: 'companyName', 3: 'registrationDate',
        },
    }
    ...
    OCR_VERIFY_CHAINS = {
        ...
        'MSME': 'MSME_UDYAM_REG_NUMBER',
        'CIN': 'CIN_ID',
        # Aadhaar types don't have a verification API - no chain
        'AADHAR_IMAGE': None,
    ...
    # EXT_DROP_DOWN field patterns - be more specific
    EXT_DROPDOWN_FIELDS = {
        'company code': 'COMPANY_CODE',
        'purchase organization': 'PURCHASE_ORGANIZATION',
        'account group': 'VENDOR_TYPES',
        'vendor type': 'VENDOR_TYPES',
        'withholding tax type': 'WITHHOLDING_TAX',
    ...
    # EXT_VALUE field patterns - be more selective
    EXT_VALUE_FIELDS = {
        'company name': 'COMPANY_NAME',
        'group key': 'CORPORATE_GROUP',
    ...
    # CONVERT_TO patterns
    CONVERT_TO_PATTERNS = [
        (r'upper\s*case|uppercase|capital\s*letters?', 'UPPER_CASE'),
        (r'lower\s*case|lowercase', 'LOWER_CASE'),
        (r'proper\s*case|title\s*case|capitalize', 'PROPER_CASE'),
        (r'trim|remove\s*spaces', 'TRIM'),
        (r'alphanumeric\s*only', 'ALPHANUMERIC'),
    ]

    # Validation patterns
    VALIDATION_PATTERNS = [
        (r'file\s*size\s*(?:should\s+be\s+)?(?:up\s+to|max|limit)\s*(\d+)\s*mb', 'FILE_SIZE'),
        (r'email\s*validation|validate\s*email|valid\s*email', 'EMAIL'),
        (r'mobile\s*validation|validate\s*mobile|valid\s*mobile', 'MOBILE'),
        ...
        (r'pincode\s*validation|pin\s*code\s*format|valid\s*pincode', 'PINCODE'),
        (r'duplicate\s*check|check\s*duplicate|de-?duplication', 'DUPLICATE'),
    ]
    ...
    def __init__(
        ...
        self.panels: Dict[str, int] = {}  # Panel name -> panel field ID
        ...
        for doc_type in doc_types:
            ...
            for meta in metadatas:
                ...
                if 'rulecheck' in field_name.lower():
                    ...
                    self.rulecheck_field = field
                    self.fields.append(field)
                    self.field_by_id[field_id] = field
                    self.field_by_name[field_name] = field
                    self.field_by_name_lower[field_name.lower()] = field
                    continue
    ...
    def match_field(self, name: str, panel: Optional[str] = None) -> Optional[FieldInfo]:
        ...
        for field_name_lower, field in self.field_by_name_lower.items():
    ...
    def process(self) -> Dict:
        ...
        self.log("Starting enhanced rule extraction v8...")

        # Step 1: Generate EXT_DROP_DOWN rules
        ...
        # Step 2: Generate EXT_VALUE rules (more selective)
        ...
        # Step 3: Generate visibility rules
        ...
        # Step 5: Generate VERIFY rules with proper destinationIds
        ...
        # Step 13: Generate special rules (COPY_TXNID, SET_DATE, etc.)
    ...
    def _generate_ext_value_rules(self):
        """Generate EXT_VALUE rules - more selective than v7."""
        for field in self.fields:
            name_lower = field.name.lower()
            logic = (field.logic or '').lower()

            # Skip if already has a verify rule destination marker
            if 'data will come from' in logic and 'validation' in logic:
                continue

            # Skip if it's a dropdown field
            if field in self.dropdown_fields:
                continue

            # Check EXT_VALUE patterns
            for pattern, ev_type in self.EXT_VALUE_FIELDS.items():
                if pattern in name_lower:
                    # Check logic for external data indicators
                    if any(x in logic for x in ['derive', 'external', 'table', 'reference', 'based on', 'column']):
                        rule = self._create_rule(
    ...
    def _generate_visibility_rules(self):
        """Generate visibility and mandatory rules."""
        if not self.intra_panel_refs:
            return
        ...
        for field in self.file_upload_fields:
            ...
            for pattern, (ocr_source, verify_dest_pattern) in self.OCR_FIELD_PATTERNS.items():
                if re.search(pattern, combined, re.I):
                    ...
                    dest_ids = self._get_ocr_destination_ids(field, ocr_source, verify_dest_pattern)
    ...
    def _get_ocr_destination_ids(self, ocr_field: FieldInfo, ocr_source: str, hint: Optional[str]) -> List[int]:
        ...
        For OCR, destinationIds contains the field IDs to populate with extracted data.
        """
        dest_ids = []
    ...
    def _generate_verify_rules(self):
        ...
        CRITICAL FIX v8: destinationIds array is indexed by ordinal (1 = index 0).
        The VALUE at each position is the FIELD ID, NOT the ordinal number.
    ...
    def _generate_verify_rule(self, source_field: FieldInfo, verify_source: str):
        ...
        CRITICAL FIX v8: destinationIds contains FIELD IDs at ordinal positions.
        """
    ...
    def _get_verify_destination_ids(self, source_field: FieldInfo, verify_source: str) -> List[int]:
        ...
        CRITICAL: Returns array indexed by ordinal, with FIELD IDs as values.
                    ...
                    for r in ref.get('references', []):
                        ...
                        if is_verify_dest:
                            ...
                            if ordinal and 1 <= ordinal <= num_ordinals:
                                ...
                                if dest_field:
                                    # CRITICAL: Store FIELD ID, not ordinal
    ...
    def _generate_disabled_rules(self):
        ...
        # Create consolidated rule on RuleCheck field (or first field)
        source_field = self.rulecheck_field or self.fields[0] if self.fields else None
        if not source_field:
            return
    ...
    def _generate_convert_to_rules(self):
        """Generate CONVERT_TO rules for text formatting."""
        for field in self.fields:
            if field.field_type not in ['TEXT', 'EXTERNAL_DROP_DOWN_VALUE']:
                continue

            logic = (field.logic or '').lower()

            for pattern, convert_type in self.CONVERT_TO_PATTERNS:
                if re.search(pattern, logic, re.I):
                    rule = self._create_rule(
                        action_type='CONVERT_TO',
                        source_ids=[field.id],
                        destination_ids=[],
                        source_type=convert_type,
                    )
                    self._add_rule(field.id, rule)
                    break
    ...
    def _generate_validation_rules(self):
        ...
        for field in self.fields:
            ...
            for pattern, val_type in self.VALIDATION_PATTERNS:
                match = re.search(pattern, logic, re.I)
                if match:
                    params = {}
                    if val_type == 'FILE_SIZE' and match.groups():
                        size_mb = match.group(1)
                        params = {'maxSize': int(size_mb) * 1024 * 1024}
    ...
    def _generate_copy_to_rules(self):
        ...
        copy_patterns = [
            (r'copy\s+(?:to|from)\s+([^,\.]+)', 'COPY_TO'),
            (r'same\s+as\s+([^,\.]+)', 'COPY_TO'),
            (r'populate\s+from\s+([^,\.]+)', 'COPY_TO'),
        ]

        for field in self.fields:
            logic = (field.logic or '').lower()

            for pattern, action in copy_patterns:
                match = re.search(pattern, logic, re.I)
                if match:
                    source_name = match.group(1).strip()
                    source_field = self.match_field(source_name, field.panel_name)

                    if source_field and source_field.id != field.id:
                        rule = self._create_rule(
                            action_type=action,
                            source_ids=[source_field.id],
    ...
    def _generate_document_rules(self):
        ...
        for field in self.file_upload_fields:
            ...
            # Generate UNDELETE_DOCUMENT rule
    ...
    def _generate_special_rules(self):
        ...
        txn_id_field = self.match_field('Transaction ID') or self.match_field('Search term')
        if txn_id_field:
            rule = self._create_rule(
    ...
    def _rebuild_rules_by_field(self):
        """Rebuild rules_by_field mapping after all rules are generated."""
        self.rules_by_field.clear()
        for rule in self.all_rules:
            for source_id in rule.source_ids:
                if source_id in self.field_by_id:
    ...
    def save_output(self, output_path: str):
        """Run extraction and save to output file."""
    ...
    def get_extraction_report(self) -> Dict:
        ...
        return {
            ...
            "rules_by_type": dict(rule_counts),
            "ocr_rules": list(self.ocr_rules.keys()),
            "verify_rules": list(self.verify_rules.keys()),
...
def main():
    """Main entry point for rule extraction v8."""
    import argparse

    parser = argparse.ArgumentParser(description='Enhanced Rule Extraction Agent v8')
    parser.add_argument('--schema', required=True, help='Path to schema JSON')
    parser.add_argument('--intra-panel', help='Path to intra-panel references JSON')
    parser.add_argument('--bud', help='Path to BUD document')
    ...
    agent = EnhancedRuleExtractionAgentV8(
    ...
    populated = agent.save_output(args.output)

    if args.report:
        report = agent.get_extraction_report()
    ...
    print(f"Generated {len(agent.all_rules)} rules")
