# Inter-Panel Dispatcher — Detailed Implementation Plan (v3)

Based on `implementation_v2.md`. Incorporates review discussion: batch splitting safety clarification, EDV-aware coverage metrics, regex hardening, and minor scope corrections.

**Changes from v2:**
- Phase 1.2: Added explicit safety note — batch splitting does NOT break Phase B clearing logic because inter-panel clearing rules are self-contained cross-panel pairs; intra-panel clearing chains are Stage 5's responsibility
- Phase 2.1.1: Regex `r'"(_[a-z0-9_]+_)"'` changed to `r'"(_[a-zA-Z0-9_]+_)"'` to handle potential uppercase variable names
- Phase 2.2: Coverage percentage denominator now excludes EDV refs (they were silently inflating "covered" count since reconciliation skips them without verification)
- Phase 2.2: Removed incorrect note about `pre_edv_filter` needing to be saved — it already persists in function scope (Python local variable at line 945)
...
- **Why batch splitting is safe for Phase B clearing logic:** The inter-panel dispatcher only creates cross-panel rules. Each cross-panel derivation ref (A in Panel 1 -> B in Panel 2) independently produces its own clearing pair (clear B when A changes). There are no multi-hop chains to trace in the inter-panel context — any downstream intra-panel clearing chains (B -> C, B -> D within Panel 2) are already handled by Stage 5 (intra-panel expression rules). Therefore, no batch needs context from another batch's output, and Phase B's chain tracing (Step 6) won't miss relationships across batches.
...
**Why `re.findall(r'"(_[a-zA-Z0-9_]+_)"', expr)` works:**
- Variable names in expressions always appear as `"_underscored_"` strings
...
- Uses `[a-zA-Z0-9_]` instead of `[a-z0-9_]` to guard against potential mixed-case variable names in future BUDs (current BUDs use lowercase only, but this is defensive)
...
- **Change:** Add a reconciliation section to the summary with EDV-aware metrics:
  ...
  ```
  This ensures the denominator only counts refs that Phase 2 is responsible for. EDV refs are reported separately — they are handled by Phase 4, not Phase 2, so including them in the "covered" count would silently inflate coverage (the reconciliation skips them without verification, and `len(all_refs) - len(unmatched)` would count skipped EDV refs as "covered").
...
- **Also fix:** The existing summary line `Total refs processed: {len(complex_refs)}` is misleading because `complex_refs` has been filtered (EDV refs removed) and extended (simple refs merged in) by the time the summary runs. Change to:
  ...
  ```
  Note: `pre_edv_filter` is already computed at line 945 and persists in function scope — no additional work needed to capture it.

- **Acceptance criteria:** Summary includes EDV-aware refs-to-rules-to-unmatched breakdown with accurate percentage. Existing Phase 2 counts are accurate.
...
Phase 2 (Observability)             [no deps on Phase 1 -- can parallel]
  ...
  |     |-- reconcile_refs_to_rules()  (returns unmatched + edv_skipped)
