...
${BOLD}Options:${NC}
  --schema <path>           API schema JSON for injection mode (stage 8)
...
${BOLD}Stages:${NC}
  ...
  5  Expression Rules      All expression rules in one pass   (needs: stage 4 output)
  6  Inter-Panel Rules     Handle cross-panel field references (needs: --bud, stage 5 output)
  7  Session Based         Deterministic RuleCheck insertion  (needs: --bud, stage 6 output)
  8  Convert to API        Convert to final API format        (needs: stage 7 output, --schema optional)
...
STAGE5_OUT="${OUTPUT_DIR}/expression_rules/all_panels_expression_rules.json"
STAGE6_OUT="${OUTPUT_DIR}/inter_panel/all_panels_inter_panel.json"
STAGE7_OUT="${OUTPUT_DIR}/session_based/all_panels_session_based.json"
STAGE8_OUT="${FINAL_OUTPUT}"
...
# ── Stage 5: Expression Rules ────────────────────────────────────────────
run_stage 5 "Expression Rules" \
    python3 "${DISPATCHERS}/expression_rule_dispatcher.py" \
        --input "$STAGE4_OUT" \
        --output "$STAGE5_OUT"

# ── Stage 6: Inter-Panel Rules ───────────────────────────────────────────
run_stage 6 "Inter-Panel Rules" \
    python3 "${DISPATCHERS}/inter_panel_dispatcher.py" \
        --clear-child-output "$STAGE5_OUT" \
        --bud "$BUD_DOC" \
        --output "$STAGE6_OUT"

# ── Stage 7: Session Based ──────────────────────────────────────────────
...
[[ -n "$API_SCHEMA" ]] && STAGE8_ARGS+=(--schema "$API_SCHEMA")
[[ -n "$PRETTY_FLAG" ]] && STAGE8_ARGS+=($PRETTY_FLAG)

run_stage 8 "Convert to API Format" "${STAGE8_ARGS[@]}"
