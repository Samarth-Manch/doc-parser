...
#
# Runs all 7 dispatcher stages sequentially, each building on the previous output.
...
${BOLD}Options:${NC}
  ...
  --start-stage <1-7>       Start from this stage (default: 1)
  --end-stage <1-7>         Stop after this stage (default: 7)
...
${BOLD}Stages:${NC}
  ...
  6  Session Based         Inject RuleCheck session rules     (needs: --bud, stage 5 output)
  7  Convert to API        Convert to final API format        (needs: stage 6 output, --schema optional)
...
STAGE6_OUT="${OUTPUT_DIR}/session_based/all_panels_session_based.json"
STAGE7_OUT="${FINAL_OUTPUT}"
...
[[ -n "$API_SCHEMA" ]] && STAGE7_ARGS+=(--schema "$API_SCHEMA")
[[ -n "$PRETTY_FLAG" ]] && STAGE7_ARGS+=($PRETTY_FLAG)

run_stage 7 "Convert to API Format" "${STAGE7_ARGS[@]}"
