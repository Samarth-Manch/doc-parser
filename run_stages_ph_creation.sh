...
BUD="documents/PH Creation new BUD_V7.docx"
...
with open('archive/output/complete_format/ph_creation-schema.json', 'w') as f:
    json.dump(schema, f, indent=2, ensure_ascii=False)
n = len(schema['template']['documentTypes'][0]['formFillMetadatas'])
print(f'  Extracted {n} fields -> archive/output/complete_format/ph_creation-schema.json')
"
...
run_stage 8 "Convert to API Format" \
    python3 dispatchers/agents/convert_to_api_format.py --schema archive/output/complete_format/ph_creation-schema.json --rules "${OUT_DIR}/inter_panel/all_panels_inter_panel.json" --output "${OUT_DIR}/test_merged_ph_creation.json" --pretty

run_stage 9 "Fix Mandatory / Editable Fields" \
    python3 dispatchers/agents/fix_mandatory_fields.py --bud "${BUD}" --json "${OUT_DIR}/test_merged_ph_creation.json"

run_stage 10 "Resolve EDV Variable Names" \
    python3 dispatchers/agents/resolve_edv_varnames.py --json "${OUT_DIR}/test_merged_ph_creation.json"

run_stage 11 "Post Trigger Rule IDs" \
    python3 dispatchers/agents/post_trigger_linker.py --json "${OUT_DIR}/test_merged_ph_creation.json"
